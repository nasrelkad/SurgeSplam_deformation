...
config = dict(
    ...
    distance_current_frame_prob=0.25,#0.1, # Probability of choosing the current frame in mapping optimization
    mapping_window_size=-1,#-1 # Mapping window size
    ...
    depth = dict(
        ...
        use_xyzt=True,
        graph = dict(
        num_nodes = 256,   # 128–512 typical
        K = 6,             # neighbors per Gaussian
        ...
        max_vel_xyz=0.05,         # scene units / frame (tight)
        max_ang_vel=0.6,         # rad / frame (~8.6°/frame)
        max_logscale_vel=0.03,    # per frame
        gates=dict(
            use_endo4dgs=True,
            endo4dgs=dict(
                normalize=True,
    ...
    GRN = dict(
        ...
        random_initialization_lrs = dict(
            ...
            node_s0=          0.00005,
            node_t0=          0.0001,
            fourier_xyz_cos=  0.00005,
            fourier_xyz_sin=  0.00005,
            fourier_ang_cos=  0.00002,
